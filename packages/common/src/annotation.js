...
var annotation_json_1 = __importDefault(require("../schemas/annotation.json"));
/**
 ...
 * @returns {object}
 */
var mock = function (input) { return kit.expandWithPaths(path, __assign({ namespace: 'test_suite', uuid: '423' }, input)); };
 ...
 * @returns {string}
 */
var makeNamespacePath = function (namespace, key) { return "(annotations[?type=='" + namespace + "'].payload" + (key ? "." + key : '') + ")[0]"; };
/**
 * List of matchers that can pull data from common annotations
 */
var publicMatcher = {
    /** Path for determining if an event is visible or not */
    hidden: makeNamespacePath(publicNamespace.visibility, publicKey.hidden),
    important: makeNamespacePath(publicNamespace.visibility, publicKey.important),
    note: makeNamespacePath(publicNamespace.notes, publicKey.note)
};
...
exports["default"] = __assign(__assign({ path: path, mock: mock, make: make, schema: annotation_json_1["default"], get: get }, customExports), { getPayload: getPayload, getPayloadKey: getPayloadKey, getNamespace: getNamespace, getUuid: getUuid, label: label, group: group, parentDepth: parentDepth });
