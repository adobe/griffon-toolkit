  ...
  if (items && items.length) {
    items.forEach(function (item) {
      ...
      if (!hasOwnProp.call(item, 'longname')) {
        ...
        const depth = (item.longname.match(/\./g) || []).length;

        itemsNav += '<li id="' + item.longname.replace('/', '_').replace(/\./g, '-') + '-nav" class="nav-depth' + depth + '">' + linktoFn(item.longname, item.name.replace(/^module:/, ''));
        if (methods.length) {
        ...
        if (item.children) {
          ...
          item.children.forEach(function (child) {
            itemsNav += '<li data-type="method" id="' + child.name + '-nav">';
            itemsNav += linktoFn(child.longname, child.name);
            itemsNav += '</li>';
          });
